...
// Ovewview counts for core entities
router.get("/ovewview", async (req, res) => {
  try {
    const [
      usersCount,
      teachersCount,
    ...
    res.status(200).json({
      data: {
        ...
        department: departmentsCount[0]?.count ?? 0,
        classes: classesCount[0]?.count ?? 0,
      }
    })
  } catch (error) {
    console.error("GET /stats/ovewview error:", error)
...
router.get("/charts", async (req, res) => {
  try {
    ...
    console.error("GET /status/charts error:", error)
    res.status(500).json({ error: "Failed to fetch charts stats" })
  }
})
