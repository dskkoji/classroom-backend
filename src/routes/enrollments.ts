...
router.post("/", async (req, res) => {
  try {
    ...
    if (!createdEnrollment) return res.status(500).json({ error: "Failed to create enrollment" })
...
router.post("/join", async (req, res) => {
  try {
    ...
    if (!inviteCode || !studentId) {
      return res.status(400).json({ error: "invideCode and studentId are required" })
    }
    ...
    const enrollment = getEnrollmentDetails(createdEnrollment.id)
